...
?>
            ...
            <div class="row">
               ...
               Çalışmalarım
                ...
                <div class="box-body">
                    ...
                    <tbody>
                    <?php $calısmalarım = $db->query("SELECT * FROM calısmalar")->fetchAll(PDO::FETCH_OBJ);
                            foreach ($calısmalarım as $row) { ?>
                            <tr>
                                ...
                                <td>
                                <a href="?sil=<?php echo $row->id ?>"><i class="fas fa-trash-alt"></i></a>
                                <a href="?düzenle=<?php echo $row->id ?>"><i class="fas fa-pen"></i></a>
                                <a href="?incele=<?php echo $row->id ?>"><i class="fas fa-clipboard-check"></i></a>
    ...
    //update  
        if(@$_GET["sil"]){
           $sil = $db->prepare("DELETE FROM calısmalar WHERE id =: id");
           $sil->execute([
               "id"=>$_GET["sil"]
           ]);
           if($sil){
               echo "Silme işleminiz başarılı";
               header("Location: calısmalarım.php");
           }
           else{
